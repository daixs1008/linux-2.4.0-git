...
struct block_device *bdget(dev_t dev)
{
	...
	spin_lock(&bdev_lock);
	bdev = bdfind(dev, head);  //在杂凑表里寻找，如果找到就返回，
	spin_unlock(&bdev_lock);
	if (bdev)
		return bdev;
	new_bdev = alloc_bdev();  //如果找不到就分配一个
	if (!new_bdev)
		return NULL;
	...
	new_bdev->bd_op = NULL;
	spin_lock(&bdev_lock);
	bdev = bdfind(dev, head);  //由于分配时可能睡眠，所以在分配后在寻找一次，如果别的进程已经成功分配，这次新分配的就撤销
	if (!bdev) {
...
int blkdev_get(struct block_device *bdev, mode_t mode, unsigned flags, int kind)
{
	...
	down(&bdev->bd_sem);
	if (!bdev->bd_op)
		bdev->bd_op = get_blkfops(MAJOR(rdev));//根据设备主设备号，设置bdev->bd_op，
	if (bdev->bd_op) {
		/*
