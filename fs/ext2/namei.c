					     ...
					     struct ext2_dir_entry_2 ** res_dir)
{
	...
	*res_dir = NULL;
	...
	toread = 0;
	for (block = 0; block < NAMEI_RA_SIZE; ++block) {  //预读内容
...
{
	struct inode * inode = ext2_new_inode (dir, mode);  //分配一个新的结构
	int err = PTR_ERR(inode);
	...
	init_special_inode(inode, mode, rdev);  //设置inode结构 包括将设备号写入该数据结构
	err = ext2_add_entry (dir, dentry->d_name.name, dentry->d_name.len,   //将新创建的节点加进所在目录在磁盘上的目录文件中
			     inode);
	if (err)
		goto out_no_entry;
	mark_inode_dirty(inode);   //将新创建的inode结构设置成脏
	d_instantiate(dentry, inode); //将内存中目录项dentry结构与inode结构间相互挂钩
	return 0;
