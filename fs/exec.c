...
int flush_old_exec(struct linux_binprm * bprm)
{
	 ...
	 */
	oldsig = current->sig;
	retval = make_private_signals();  //只是检查一下共享计数就返回了
	if (retval) goto flush_failed;

	/* 
	 * Release all of the old mmap stuff
	 */
	retval = exec_mmap();  //从父进程继承的用户空间就是在这里放弃的
...
#endif
	for (try=0; try<2; try++) { //共循环两次，为了在安装了模块以后再来试一次
		read_lock(&binfmt_lock);
		for (fmt = formats ; fmt ; fmt = fmt->next) {  //让formats队列中的成员诸葛试试他们的 load_binary()函数，看看能不能打开
...
int do_execve(char * filename, char ** argv, char ** envp, struct pt_regs * regs)
{
	...
	file = open_exec(filename);  //将给定的可执行程序找到并打开
	...
	bprm.sh_bang = 0;  //现在还不知道文件的性质，暂且设定为二进制
	bprm.loader = 0;
	bprm.exec = 0;
	...
	retval = search_binary_handler(&bprm,regs);  //内核中有个队列formats，挂在此队列中的成员是代表着各种可执行文件格式的“代理人”每个成员只认识并且处理一种特定的格式的可执行文件的运行
	if (retval >= 0)
		/* execve success */
		return retval;
