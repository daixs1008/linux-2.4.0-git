...
{
	struct super_block * s;
	s = get_empty_super();  //从队列中找到一个空闲的super_block结构
	if (!s)
		goto out;
	s->s_dev = dev;  //进行初始化
	s->s_bdev = bdev;
	...
	char *dev_name, int flags, void * data)
{
	...
	error = -ENOTBLK;
	if (!S_ISBLK(inode->i_mode))  //找到的inode结构必须代表着一个块设备，其i_mode的S_ISBLK标志必须为1，
		goto out;
	error = -EACCES;
	if (IS_NODEV(inode))
		goto out;
	bdev = inode->i_bdev;
	bdops = devfs_get_ops ( devfs_get_handle_from_inode (inode) );//支持两种设备方案
	if (bdops) bdev->bd_op = bdops;  //块设备驱动的跳转表
	/* Done with lookups, semaphore down */
	down(&mount_sem);
	dev = to_kdev_t(bdev->bd_dev);
	sb = get_super(dev);  //寻找超级快 ，大多数情况都会失败，因而得到从设备读入其超级快并在内存中建立起设备的super_block数据结构，为了这个目的，先得打开这个设备
	if (sb) {
		...
		if (!(flags & MS_RDONLY))
			mode |= FMODE_WRITE;
		error = blkdev_get(bdev, mode, 0, BDEV_FS); //打开设备
		if (error)
			goto out;
		check_disk_change(dev);
		error = -EACCES;
		if (!(flags & MS_RDONLY) && is_read_only(dev))  //安装检查，如果是只读，那就不能安装
			goto out1;
		error = -EINVAL;
		sb = read_super(dev, bdev, fs_type, flags, data, 0);  //从设备上读入超级快并在内存中建立super_block
		if (sb) {
		  ...
		  unsigned long flags, void *data_page)
{
	...
	if (!dir_name || !*dir_name || !memchr(dir_name, 0, PAGE_SIZE))
		return -EINVAL;
	if (dev_name && !memchr(dev_name, 0, PAGE_SIZE))  //memchr()在指定长度的缓冲区寻找指定的字符，如果找不到就返回0，
		return -EINVAL;

	/* OK, looks good, now let's see what do they want */

	/* just change the flags? - capabilities are checked in do_remount() */
	if (flags & MS_REMOUNT)  // 如果调用参数中的MS_REMOUNT标志位为1，就表示所要求的只是改变一个原已安装的设备的安装方式，
	...
	if (fstype->fs_flags & FS_NOMOUNT)  //有些虚拟的文件系统，要有kern_mount() 安装，这里会出错返回
		sb = ERR_PTR(-EINVAL);
	else if (fstype->fs_flags & FS_REQUIRES_DEV)  //一般正常的文件如：ext2,minix,ufs等，通过get_sb_bdev读入超级块
		sb = get_sb_bdev(fstype, dev_name, flags, data_page);
	else if (fstype->fs_flags & FS_SINGLE)  //有些虚拟文件系统在安装了同类型中的第一个设备，从而创建了超级快的super_block数据以后，再安装同同类中的其他设备就共享已经存在的超级快
		sb = get_sb_single(fstype, flags, data_page);
	else
		sb = get_sb_nodev(fstype, flags, data_page); //get_sb_nodev生成一个超级快
			  ...
			  unsigned long flags, void * data)
{
	...
	dir_page = getname(dir_name);  //待安装点的路径  ----dir_name 安装点的路径名
