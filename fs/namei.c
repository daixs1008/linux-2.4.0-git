...
static inline int do_getname(const char *filename, char *page)
{
	...
	if ((unsigned long) filename >= TASK_SIZE) {  //TASK_SIZE -->3GB (default).
		if (!segment_eq(get_fs(), KERNEL_DS))
			return -EFAULT;
	} else if (TASK_SIZE - (unsigned long) filename < PAGE_SIZE)
		len = TASK_SIZE - (unsigned long) filename;

	retval = strncpy_from_user((char *)page, filename, len);  //执行具体的拷贝工作
	if (retval > 0) {
		if (retval < len)
			return 0;
		return -ENAMETOOLONG;
...
char * getname(const char * filename)
{
	...
	tmp = __getname();  //分配一个页面作为缓冲   --  为什么要分配页面，原因1，这个字符串可能很长（是一个绝对路径），原因2，进程系统空间堆栈的大小是大约7KB，不能滥用
	if (tmp)  {           //完成以后还要通过 putname() 释放
		int retval = do_getname(filename, tmp);  //从用户空间拷贝字符串
...
int path_walk(const char * name, struct nameidata *nd)
{
	...
	for(;;) {
		...
		if (nd->dentry->d_op && nd->dentry->d_op->d_hash) {
			err = nd->dentry->d_op->d_hash(nd->dentry, &this);  //计算杂凑值
			if (err < 0)
				break;
		}
...
int path_init(const char *name, unsigned int flags, struct nameidata *nd)
{
	...
	read_lock(&current->fs->lock);
	nd->mnt = mntget(current->fs->pwdmnt);  // current->fs->pwdmnt 指向一个vfsmount 结构，这个结构包含有关设备信息，和安装点的信息
	nd->dentry = dget(current->fs->pwd);  //current->fs->pwd 指向当前目录的dentry 结构，表示在虚拟的绝对路径中的这个节点及所有在此之前的节点都解决了。
	read_unlock(&current->fs->lock);
	return 1;
}
 ...
 * SMP-safe.
 *///这个函数先在dentry杂凑队列里寻找，找不到就先创建一个新的dentry数据结构，在到目标文件所在的目录中寻找一次，如果找到了就把已经创建的dentry归还，找不到才采取他
...
static inline int may_create(struct inode *dir, struct dentry *child) {
	if (child->d_inode)
		return -EEXIST;
	if (IS_DEADDIR(dir)) //检查目标文件所在的目录是否实际上已被删除
		return -ENOENT;
...
//参数dir指向所在目录的inode结构，而dentry则指向待创建文件的dentry。
int vfs_create(struct inode *dir, struct dentry *dentry, int mode)
{
	int error;

	mode &= S_IALLUGO & ~current->fs->umask; //每个进程都有个“文件访问权限屏蔽”umask，如果umask某位为1，则由此进程所创建的文件就把相应的访问权限屏蔽掉。
	mode |= S_IFREG;

	down(&dir->i_zombie);//防止受到干扰，
...
int open_namei(const char * pathname, int flag, int mode, struct nameidata *nd)
{
	 ...
	 */
	if (!(flag & O_CREAT)) {  //O_CREAT标志位表示如果要打开的文件不存在就创建这个文件
	 ...
	 */
	if (path_init(pathname, LOOKUP_PARENT, nd))  //创建，我们需要知道parent节点
		error = path_walk(pathname, nd);
	if (error)
		return error;
	 ...
	 */
	error = -EISDIR;
	if (nd->last_type != LAST_NORM || nd->last.name[nd->last.len])  //open只能创建文件不能创建目录，同时目标节点名必须是以“\0”结尾的，那才是正常的文件名
		goto exit;

	dir = nd->dentry;  //我们已经找到目标文件所在目录的dentry结构，并让指针dir指向这个数据结构
	down(&dir->d_inode->i_sem);
	dentry = lookup_hash(&nd->last, nd->dentry); //寻找目标文件的dentry结构
...
do_last:
	...
	if (!dentry->d_inode) {
		error = vfs_create(dir->d_inode, dentry, mode); //创建dentry
		up(&dir->d_inode->i_sem);
		dput(nd->dentry);
		nd->dentry = dentry;
		if (error)
...
ok:
	 ...
	 */
	error = get_lease(inode, flag);  //这与文件的租借 有关
	if (error)
		goto exit;

	if (flag & O_TRUNC) {
		error = get_write_access(inode); // inode 中有个计数器 i_write_count 用来对正在写访问该文件的进程计数，这个数为负值可以写操作，正值不可写操作
		if (error)
			goto exit;
		...
		if (!error) {
			...
			error = do_truncate(dentry, 0); //将文件中原有的内容全部删除，成为“截尾”
		}
...
do_link:
	...
	if (error)
		return error;
	if (nd->last_type == LAST_BIND) {  //只发生在 /proc 或类似的特殊文件系统中
		dentry = nd->dentry;
		goto ok;
	}
	error = -EISDIR;
	...
	dir = nd->dentry;
	down(&dir->d_inode->i_sem);
	dentry = lookup_hash(&nd->last, nd->dentry); //找到或创建真实发的目标节点的dentry结构
	putname(nd->last.name);
	goto do_last; //前面对目标节点的一系列检查
}
...
int vfs_mknod(struct inode *dir, struct dentry *dentry, int mode, dev_t dev)
{
	...
	down(&dir->i_zombie);
	if ((S_ISCHR(mode) || S_ISBLK(mode)) && !capable(CAP_MKNOD))  //权限检查的第一步（共两步）
		goto exit_lock;

	error = may_create(dir, dentry);  //权限检查的第二步（共两步）
	if (error)
		goto exit_lock;
...
asmlinkage long sys_mknod(const char * filename, int mode, dev_t dev)  //dev就是设备号
{
	int error = 0;
	char * tmp;
	struct dentry * dentry;
	...
	if (path_init(tmp, LOOKUP_PARENT, &nd))  //找打待安装节点的父节点
	...
	if (!IS_ERR(dentry)) {
		...
		case S_IFCHR: case S_IFBLK: case S_IFIFO: case S_IFSOCK:
			error = vfs_mknod(nd.dentry->d_inode,dentry,mode,dev);  //创建特殊文件，包括：FIFO，插口，字符设备，块设备文件
			break;
		case S_IFDIR:
			error = -EPERM;
			break;
