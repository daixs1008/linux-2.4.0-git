...
struct file *filp_open(const char * filename, int flags, int mode)
{
	...
	if (!error)
		return dentry_open(nd.dentry, nd.mnt, flags);  //建立目标文件的上下文，即file数据结构，并让它与当前进程的task_sturct结构挂上钩，成为该文件在当前进程task_struct结构中的一个代表
...
struct file *dentry_open(struct dentry *dentry, struct vfsmount *mnt, int flags)
{
	...
	error = -ENFILE;
	f = get_empty_filp(); //分配一个空闲的file 结构
	if (!f)
		goto cleanup_dentry;
	f->f_flags = flags;
	...
	if (f->f_mode & FMODE_WRITE) {
		error = get_write_access(inode); //一方面检擦权限，一方面递增 i_writecount,保证互斥
		if (error)
			goto cleanup_file;
	}
	...
	f->f_pos = 0;
	f->f_reada = 0;
	f->f_op = fops_get(inode->i_fop); //是个宏操作
	if (inode->i_sb)
	...
	f->f_flags &= ~(O_CREAT | O_EXCL | O_NOCTTY | O_TRUNC);//最后这些标志位已经不再需要了，因为它们只是对打开文件有作用，而现在已经打开了，所以把这些标志清0，
...
asmlinkage long sys_open(const char * filename, int flags, int mode)  //mode 表示打开的模式
{
	char * tmp;
	int fd, error;
	...
	flags |= O_LARGEFILE;
#endif
	tmp = getname(filename);  //filename文件的路径名  --  //从用户空间把文件的路径名拷贝到系统空间
	fd = PTR_ERR(tmp);
	if (!IS_ERR(tmp)) {
		fd = get_unused_fd();  //从当前进程的“打开文件表”中找一个空闲的表项，该表项的下标就是打开文件号
		if (fd >= 0) {
			struct file *f = filp_open(tmp, flags, mode); //找到或者创建一个“链接”，或者说读写该文件的上下文，文件读写的上下文是由file数据结构代表和描绘的
			error = PTR_ERR(f);
			if (IS_ERR(f))
				goto out_error;
			fd_install(fd, f); //将指向这个结构的指针填入当前进程的打开文件表，即由其task_sturct结构中的指针file所指向的files_struct数组中并返回其数组的下标。
		}
out:
		putname(tmp);
	}
...
int filp_close(struct file *filp, fl_owner_t id)
{
	...
	locks_remove_posix(filp, id);  //试一下，有没有忘记在关闭前把锁解除
	fput(filp);
	return retval;
}
...
asmlinkage long sys_close(unsigned int fd)
{
	...
	if (fd >= files->max_fds)
		goto out_unlock;
	filp = files->fd[fd];
	if (!filp)  //检查指针不为0，原因看毛德操老师Linux--p577
		goto out_unlock;
	files->fd[fd] = NULL;
	FD_CLR(fd, files->close_on_exec); //将位图清0，
