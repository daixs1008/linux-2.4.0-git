...
static inline void kmem_cache_free_one(kmem_cache_t *cachep, void *objp)
{
	...
	/* fixup slab chain */
	if (slabp->inuse-- == cachep->num)  //递减所属slab队列控制结构中非空闲对象的计数   ---//递减后有三种可能
		goto moveslab_partial;
	if (!slabp->inuse)
		goto moveslab_free;
	return;
                   //缓冲区的释放并不导致slab的释放，空闲slab的释放是由kswapd等内核线程周期的调用kmem_cache_reap() 完成的
moveslab_partial:
    	/* was full.
 ...
 */
void kmem_cache_reap (int gfp_mask)  //扫描slab队列的队列cache_cache,从中发现可工收割的slab队列
{                                            //不过每次不是全部扫描，而是扫描一部分，
	slab_t *slabp;                           //找打可收割的slab也不是全部回收，而是回收大约 80%
