	...
	unsigned long address, int write_access)
{
	...
	pgd = pgd_offset(mm, address);  //pgd_offset(mm, address) ((mm)->pgd+pgd_index(address)) //计算指向该地址所属页面目录项的指针，
	pmd = pmd_alloc(pgd, address);  //return (pmd_t *) pgd;  因为i386 只使用两层映射，所以直接返回

	if (pmd) {  //页面目录总是存在，相应的目录项也许已经指向一个页面表，此时需要根据给定的地址在表中找到相应的页面表项，
		pte_t * pte = pte_alloc(pmd, address); //或者，目录项也可能是空的，那样就需要先分配一个页面表，再在页面表中找到相应的表项，这样才可以为下面分配
		if (pte)                                //物理内存页面并建立映射做好准备
			ret = handle_pte_fault(mm, vma, address, write_access, pte);  //如果映射所需的的“基础设施”都已经齐全了，但页面表项PTE还是空的，剩下的就是物理内存页面本身了，
	}
	return ret;
}
