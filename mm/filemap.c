...
{
	struct page *page, **hash = page_hash(mapping, index);  //找到杂凑表，
repeat:
	page = __find_lock_page(mapping, index, hash); //具体寻找目标页面是此函数完成的
	if (!page) {
		if (!*cached_page) {
			*cached_page = page_cache_alloc();//找不到就分配一个
			if (!*cached_page)
				return NULL;
		}
		page = *cached_page;
		if (add_to_page_cache_unique(page, mapping, index, hash))//加入的杂凑表
			goto repeat;
		*cached_page = NULL;
	}
	return page;
...
generic_file_write(struct file *file,const char *buf,size_t count,loff_t *ppos)
{
	...
	if (file->f_flags & O_APPEND)// O_APPEND 标志位为1，表示对此文件的写操作只能添在尾部
		pos = inode->i_size;
	...
	status  = 0;
	if (count) {  //运行到此，只要count不为0，就是一次有效的写操作，所以要在inode上打上时间印记，并把inode设成脏
		remove_suid(inode);
		inode->i_ctime = inode->i_mtime = CURRENT_TIME;
		mark_inode_dirty_sync(inode);
	}

	while (count) {  //写操作的主体部分，循环的次数取决于写的长度和位置，在每一次循环中，只往一个缓冲页面中写，并将pos的位置向前推进，
		 ...
		 */
		offset = (pos & (PAGE_CACHE_SIZE -1)); /* Within page */  //首先根据当前pos的值计算出本次循环要写的缓冲页面index,
		index = pos >> PAGE_CACHE_SHIFT;   //用右移速度快               //在该页面中的起点offset以及写入长度bytes
		...
		page = __grab_cache_page(mapping, index, &cached_page);  //计算出目标页面后，用此函数找到该缓冲页面，找不到就分配/建立一盒缓冲页面。
		if (!page)
			break;
