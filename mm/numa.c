...
#endif
	start = temp;
	while (temp) {  //函数分两段，两个while 第一段 先是从temp开始到队末，
		if ((ret = alloc_pages_pgdat(temp, gfp_mask, order))) //gfp_mask 在alloc_pages_pgdat 函数中用作 node_zonelists[] 的下标，表示具体的分配策略
			return(ret);
		temp = temp->node_next;
	}
	temp = pgdat_list;
	while (temp != start) { //第二段 然后回头从第一个节点到开始的地方
