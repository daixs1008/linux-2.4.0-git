...
void __swap_free(swp_entry_t entry, unsigned short count)
{
	...
	if (!entry.val)  //不做任何事情，因为在任何页面交换设备或文件中页面0是不用与页面交换的
		goto out;

	type = SWP_TYPE(entry); //获取页面交换的序号 ，即swap_info_struct 结构在swap_info[] 的序号，所以156行使用此下标取得具体文件的swap_info_struct 结构
	if (type >= nr_swapfiles)
		goto bad_nofile;
	p = & swap_info[type];
	if (!(p->flags & SWP_USED))
		goto bad_device;
	offset = SWP_OFFSET(entry); //获取页面在文件中的位置
	if (offset >= p->max)  //获取的位置不能大于页面的最大值
		goto bad_offset;
	if (!p->swap_map[offset]) //p->swap_map[offset] 为0，则是没有分配
		goto bad_free;
	swap_list_lock();
	if (p->prio > swap_info[swap_list.next].prio)
		swap_list.next = type;
	swap_device_lock(p);
	if (p->swap_map[offset] < SWAP_MAP_MAX) {  //同时 p->swap_map[offset] 也不能大于 SWAP_MAP_MAX
		if (p->swap_map[offset] < count)  //count 表示有几个使用者释放该页面，所以从计数中减去count
			goto bad_count;
		if (!(p->swap_map[offset] -= count)) {  //到达0时，表示该页面真正的空闲了，
			if (offset < p->lowest_bit)
				p->lowest_bit = offset;
			if (offset > p->highest_bit)
				p->highest_bit = offset;
			nr_swap_pages++;  //可供分配的盘上页面增加了
		}  //释放盘上页面的操作实际并不涉及磁盘操作，而只是内存账面上的操作，表示磁盘上那个页面的内容已经作废，所以花费的代价时很小的
	}
	swap_device_unlock(p);
	swap_list_unlock();
out:
