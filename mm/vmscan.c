...
static int try_to_swap_out(struct mm_struct * mm, struct vm_area_struct* vma, unsigned long address, pte_t * page_table, int gfp_mask)
{
	...
	pte = *page_table;  //此处page_table 实际指向页面表项，而不是页面表。
	if (!pte_present(pte))
		goto out_failed;
	page = pte_page(pte);
	...
	if (!onlist)
		...
		age_page_down_ageonly(page);  //减速页面寿命
	 ...
	 */
	if (page->age > 0)  //只要页面寿命没有减到0 就不能将次页面换出，所以也要跳转到 out_failed
		goto out_failed;

	if (TryLockPage(page))  //获取锁
		goto out_failed;
	 ...
	 * bits in hardware.
	 */
	pte = ptep_get_and_clear(page_table);  //再读一次页面表的内容，并把表项的内容清0，暂时撤销该页面的映射，
	...
	if (PageSwapCache(page)) {
		...
		if (pte_dirty(pte))
			set_page_dirty(page);
set_swap_pte:
		swap_duplicate(entry);   //一是对要索引项的内容作一些检验，二是要递增相应盘上页面的共享计数
...
static int swap_out(unsigned int priority, int gfp_mask)
{
	 ...
	 */
	counter = (nr_threads << SWAP_SHIFT) >> priority;  //根据内核中进程的个数和调用swap_out 的优先级计算 counter 的值
	if (counter < 1)                          //当 priority 为0 时，counter 就等于系统的进程数
		counter = 1;
...
int page_launder(int gfp_mask, int sync)
{
	...
	launder_loop = 0;   //如果有必要进行第二次扫描，将会设成1，--->684行
	maxlaunder = 0;
	cleaned_pages = 0;

dirty_page_rescan:
	spin_lock(&pagemap_lru_lock);
	maxscan = nr_inactive_dirty_pages;  //避免重复扫描同一页面，甚至陷入死循环
	while ((page_lru = inactive_dirty_list.prev) != &inactive_dirty_list &&    //对不活跃“脏”页面队列的扫描是通过while 循环进行的
				maxscan-- > 0) {                   //maxscan 防止死循环
		page = list_entry(page_lru, struct page, lru);
		...
		/* Page is or was in use?  Move it to the active list. */
		if (PageTestandClearReferenced(page) || page->age > 0 ||   //页面的寿命还没有耗尽
				(!page->buffers && page_count(page) > 1) ||    //至少有一个进程的映射表中有映射
				page_ramdisk(page)) {    //页面在受到进程用户空间映射的同时用用与ramdisk,即用内存空间来模拟磁盘，这种页面不应该换出
		 ...
		 * last copy..
		 */
		if (PageDirty(page)) {   //原则上是要将其写出到交换设备上，但有些特殊情况做处理
			int (*writepage)(struct page *) = page->mapping->a_ops->writepage;
			int result;

			if (!writepage)  //所属的address_space 数据结构必须提供页面写出的操作函数，否则就只好转到 page_active 处，将页面送回页面活跃队列
				goto page_active;
			...
			/* OK, do a physical asynchronous write to swap.  */  //在第一趟扫描中，只是把页面移到同一队列的尾部，而并不写出，如果是第二趟的话就真要写出去了。
			ClearPageDirty(page);   //把页面PG_dirty标志清成0，
			page_cache_get(page);
			spin_unlock(&pagemap_lru_lock);
		...
		if (page->buffers) {
			...
			clearedbuf = try_to_free_buffers(page, wait);  //试图释放页面
			...
			if (!clearedbuf) {                      //如果不能释放根据返回值，链入相应的列表。。或退回不活跃脏页面队列，或链入活跃页面队列，或者不活跃干净队列
				add_page_to_inactive_dirty_list(page);
...
page_active:
	 ...
	 * IO.
	 */
	if (can_get_io_locks && !launder_loop && free_shortage()) {   //完成了一趟扫描，还要根据系统中空闲页面是否短缺，以及调用参数gfp_mask 的 __GFP_IO标志是否为1，来决定是否进行第二趟扫描
		launder_loop = 1;
 ...
 */
int refill_inactive_scan(unsigned int priority, int oneshot)  //根据priority 扫描整个活跃页面的一部分，priority为0 时，扫描整个队列
{
	struct list_head * page_lru;
	struct page * page;
	...
	while (maxscan-- > 0 && (page_lru = active_list.prev) != &active_list) {
		...
		if (PageTestandClearReferenced(page)) {  //根据页面是否受到了访问，决定增加还是减少页面寿命
			 ...
			 */
			if (page->age == 0 && page_count(page) <=     //如果寿命减到0 ，还得看是否还有用户空间映射，只要页面映射大于1，
						(page->buffers ? 2 : 1)) {   //说明还有页面映射，还不能转入不活跃状态，这样的页面通过 swap_out() 扫描相应的进程映射表才能转入不活跃状态
		...
		if (page_active || PageActive(page)) {
			...
			ret = 1;
			if (oneshot)  //根据oneshot的值决定是否继续扫描
				break;
		}
	}
	spin_unlock(&pagemap_lru_lock);
...
static int refill_inactive(unsigned int gfp_mask, int user)
{
	...
	kmem_cache_reap(gfp_mask);  //收割有 slab 机制管理的空闲物理页面，相对而言，这是动作最小的

	priority = 6;     //循环先从优先级最低的6开始，逐步加大力度直到0级，结果或是达到目标，回收的数量够了，或者在最高优先级也达不到目标，那也只好算了
	do {
		made_progress = 0;

		if (current->need_resched) { //need_resched 为1 ，说明某个中断服务程序需要调度，所以调用schedule()让内核进行一次调度，但此前要把本进程状态设成TASK_RUNNING
			__set_current_state(TASK_RUNNING);
			schedule();
		}

		while (refill_inactive_scan(priority, 1)) {  //扫描活跃页面队列，试图从中找出可以转入不活跃状态的页面
			made_progress = 1;
			if (--count <= 0)
				goto done;
		}
		 ...
		 */
		while (swap_out(priority, gfp_mask)) {  //找出一个进程，然后扫描其映射表，从中找出可以转入不活跃状态的页面，
			made_progress = 1;          //swap_out 实际只是为把一些页面换出到交换设备上做好准备，而并不一定时物理意义上的页面换出
			if (--count <= 0)
				goto done;
		}
...
int kswapd(void *unused)
{
	...
	for (;;) {
		...
		if (inactive_shortage() || free_shortage()) {
			...
			if (waitqueue_active(&kswapd_done))  //看看kswapd_done 队列中是否有函数在等待执行
				wait = 1;
			do_try_to_free_pages(GFP_KSWAPD, wait);  //设法释放和换出若干页面
		}
		 ...
		 */
		if (!free_shortage() || !inactive_shortage()) {
			interruptible_sleep_on_timeout(&kswapd_wait, HZ);  //睡眠
		/*
...
static int __init kswapd_init(void)
{
	...
	swap_setup();   //更具物理内存的大小设定一个全局量page_cluster
