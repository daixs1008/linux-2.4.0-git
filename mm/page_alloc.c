...
static struct page * rmqueue(zone_t *zone, unsigned long order)
{
	...
	spin_lock_irqsave(&zone->lock, flags);
	do {        //主要操作      --它首先在下好满足大小要求的队列里分配，如果不行的话就试试更大的队列中分配，成功的话，就把分配到的大块中的剩余部分分解成小块连入相应的队列中，
		head = &area->free_list;
		...
		if (curr != head) {
			...
			page = memlist_entry(curr, struct page, list);  //从一个非空队列中取第一个结构page元素，
			if (BAD_RANGE(zone,page))
				BUG();
			memlist_del(curr);  //将元素从队列中摘除
			...
			unsigned long order, int limit, int direct_reclaim)
{
	...
	for (;;) {
		...
		if (z->free_pages + z->inactive_clean_pages > water_mark) {
			...
			if (direct_reclaim && z->free_pages < z->pages_min + 8)   //direct_reclaim  为非0 表示要求分配的一定是单个页面
				page = reclaim_page(z);
...
struct page * __alloc_pages(zonelist_t *zonelist, unsigned long order)
{
	...
	unsigned int gfp_mask = zonelist->gfp_mask;  //代表具体分配策略的数据结构，是一些用于控制目的标志位
	struct page * page;

	/*
	 * Allocations put pressure on the VM subsystem.
	 */
	memory_pressure++;  //表示内存页面管理所承受的压力，分配内存页面时递增，归还时递减，
	 ...
	 * list?
	 */    //如果要求分配的只是单个页面，而且要等待非陪完成，又不是用于管理，则把局部变量 direct_reclaim 设成 1.
	if (order == 0 && (gfp_mask & __GFP_WAIT) &&
			!(current->flags & PF_MEMALLOC))
		direct_reclaim = 1;  //表示可以从相应的页面管理区的“不活跃干净页面”缓冲队列中回收，
...
try_again:
	 ...
	 * any data ... DUH!
	 */    //这是对一个分配策略中所规定的所有页面管理区的循环。循环中一次考察各个管理区中空闲页面的总量
	zone = zonelist->zones;
	for (;;) {
		zone_t *z = *(zone++);
		if (!z)
		...
		if (z->free_pages >= z->pages_low) {  //如果总量尚在“低水位”以上，就通过rmqueue()试图从管理区中分配。
			page = rmqueue(z, order);  //试图从一个页面管理区分配若干连续的内存页面
			if (page)
				return page;
		} else if (z->free_pages < z->pages_min &&
					waitqueue_active(&kreclaimd_wait)) {   //要是发现管理区中的空闲页面总量已经降低到最低点，
				wake_up_interruptible(&kreclaimd_wait);  //而且进程(实际只能是内核线程kreclaimd)在一个等待kreclaimd_wait中睡眠，就把他唤醒，让他帮忙回收一些页面备用
		}
	}
	 ...
	 */
	page = __alloc_pages_limit(zonelist, order, PAGES_HIGH, direct_reclaim);  //先以参数PAGES_HIGH 调用 __alloc_pages_limit 
	if (page)
		return page;
	 ...
	 * succeed here.
	 */
	page = __alloc_pages_limit(zonelist, order, PAGES_LOW, direct_reclaim);  //如果以先以参数PAGES_HIGH还不行，就加大力度，用PAGES_LOW 再一次调用
	if (page)
		return page;
                              //如果到这里还没有分配页面的成功，就说明这些管理区的页面已经严重短缺了
	/*
	 ...
	 */
	wakeup_kswapd(0);
	if (gfp_mask & __GFP_WAIT) {   //如果分配策略表明对于要求分配的页面是志在必得，分配不到宁可等待，就让系统来一次调度，并且让当前进程为其他进程让出CPU
	...
	if (!(current->flags & PF_MEMALLOC)) {
		...
		if (order > 0 && (gfp_mask & __GFP_WAIT)) {
			...
			current->flags |= PF_MEMALLOC;
			page_launder(gfp_mask, 1);   //把“脏”页面洗干净
			current->flags &= ~PF_MEMALLOC;
			for (;;) {
				zone_t *z = *(zone++);
				if (!z)
				...
				while (z->inactive_clean_pages) {
					...
					if (!page)
						break;
					__free_page(page);  //释放页面时，会把空闲页面拼装成尽可能大的页面
					/* Try if the allocation succeeds. */
					page = rmqueue(z, order);  //释放页面后，在试一下，看看能否已经满足要求了
					if (page)
						return page;
				}
			}
	 ...
	 * deadlock the system...
	 */
	zone = zonelist->zones;   //到此还没有获取页面，继续加大力度
	for (;;) {
		zone_t *z = *(zone++);
		struct page * page = NULL;
		if (!z)
		...
		page = rmqueue(z, order);
		if (page)
			return page;  //到这里还没有分配一个页面，说明系统出问题了。
	}
