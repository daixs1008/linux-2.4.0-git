...
struct swap_info_struct {
	...
	unsigned short * swap_map;  //指向一个数组，（有page结构组成的数组）数组的下标决定了该页面在盘上或文件中的位置，数组的大小取决于pages，它表示该页面交换设备或文件的大小
	unsigned int lowest_bit;  //表示文件中从什么地方开始到什么地方为止时提供页面交换使用的
	unsigned int highest_bit;
	unsigned int cluster_next;  //分配盘上页面空间时尽可能按集群（cluster）方式进行
	unsigned int cluster_nr;
	int prio;			/* swap priority */
	int pages;
	unsigned long max; //表示该设备或文件中最大的页面号，也就是设备或文件的物理的大小
