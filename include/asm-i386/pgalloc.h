...
extern inline pte_t * pte_alloc(pmd_t * pmd, unsigned long address)
{
	...
	if (pmd_bad(*pmd))
		goto fix;
	return (pte_t *)pmd_page(*pmd) + address;  //相应的目录项也许已经指向一个页面表，此时需要根据给定的地址在表中找到相应的页面表项，
getnew:  //需要分配一个页面表
{
	unsigned long page = (unsigned long) get_pte_fast();  //在要分配一个页面表时，先看一下缓冲池，这就是 get_pte_fast();
	
	if (!page)   //如果缓冲池空了，就需要 get_pte_slow 分配
