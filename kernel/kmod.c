...
int exec_usermodehelper(char *program_path, char *argv[], char *envp[])
{
	...
	use_init_fs_context();  //从init_task结构中直接继承其根目录等资源
...
static int exec_modprobe(void * module_name)  //内核线程
{
	static char * envp[] = { "HOME=/", "TERM=linux", "PATH=/sbin:/usr/sbin:/bin:/usr/bin", NULL };
	char *argv[] = { modprobe_path, "-s", "-k", "--", (char*)module_name, NULL }; //如果module_name为mymodule的话，argv[]就相当于：/sbin/modprobe -s - k mymodule
	int ret;                                // -s 表示安装过程中产生的信息应写入系统日志，不要再控制中断显示。-k 表示安装时将模块的MOD_AUTOCLEAN标志位设成1
...
int request_module(const char * module_name)  //主动启动模块安装--可能嵌套
{
	pid_t pid;
	int waitpid_result;
	sigset_t tmpsig;
	int i;
	static atomic_t kmod_concurrent = ATOMIC_INIT(0); //kmod_concurrent作为嵌套深度计数器
#define MAX_KMOD_CONCURRENT 50	/* Completely arbitrary value - KAO */
	...
	pid = kernel_thread(exec_modprobe, (void*) module_name, 0);  //创建内核线程
	if (pid < 0) {
	...
	siginitsetinv(&current->blocked, sigmask(SIGKILL) | sigmask(SIGSTOP));  //创建内核线程成功后把当前线程除SIGKILL和SIGSTOP以外的所有信号都屏蔽掉免得再等待模块安装的过程中受到干扰
	recalc_sigpending(current);
	spin_unlock_irq(&current->sigmask_lock);

	waitpid_result = waitpid(pid, NULL, __WCLONE);  //这里睡眠唤醒时，内核线程exec_modprobe（） 运行已经结束了，根据其返回值判断操作是否成功
	atomic_dec(&kmod_concurrent);
	...
	if (waitpid_result != pid) {  //如果内核线程exec_modprobe（）失败，打印运行日志
