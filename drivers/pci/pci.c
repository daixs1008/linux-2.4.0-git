...
static struct pci_dev * __init pci_scan_device(struct pci_dev *temp)
{
	...
	if (pci_read_config_dword(temp, PCI_VENDOR_ID, &l)) //读出第一个长字，高16位为厂商编号，低16位为设备号
		return NULL;
	...
	if (pci_setup_device(dev) < 0) {//继续读设备信息
		kfree(dev);
		dev = NULL;
	}
	return dev;
...
struct pci_dev * __init pci_scan_slot(struct pci_dev *temp)
{
	...
	for (func = 0; func < 8; func++, temp->devfn++) {
		...
		if (pci_read_config_byte(temp, PCI_HEADER_TYPE, &hdr_type))
			continue;
		temp->hdr_type = hdr_type & 0x7f;  //读到头部类型

		dev = pci_scan_device(temp); //进一步读取具体设备的配置信息
		if (!dev)
			continue;
		pci_name_device(dev);  //将厂商信息转换成字符
		if (!func) {
		...
		pci_fixup_device(PCI_FIXUP_HEADER, dev);//读出信息后，进行修正
	}
	return first_dev;
}
...
static unsigned int __init pci_do_scan_bus(struct pci_bus *bus)
{
	...
	/* Go find them, Rover! */
	for (devfn = 0; devfn < 0x100; devfn += 8) {  //依次对各个PCI接口扫描
		dev0.devfn = devfn;
		pci_scan_slot(&dev0);
	}
...
struct pci_bus * __init pci_alloc_primary_bus(int bus)
{
	...
	if (pci_bus_exists(&pci_root_buses, bus)) {  //同一个PCI总线只能由一个pci_bus结构在PCI树中或pci_root_buses队列中只能出现一次
...
{
	struct pci_bus *b = pci_alloc_primary_bus(bus);//分配一个数据结构
	if (b) {
		b->sysdata = sysdata;
		b->ops = ops;
